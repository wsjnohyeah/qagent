...
- Local lint, strict type checking, 34 tests, API readiness, the HTTP vertical slice, and the secret scan pass.
...
- Immutable evidence packets, point-in-time feature snapshots, strategy specifications,
  ...
  feature parity checks are stored through Alembic revision `20260904_0008`.
...
- Offline/full-history and online/as-of feature materialization produce persisted comparison
  hashes. The current baseline backtester fails closed across corporate actions until the
  event-driven engine models share and cash effects.
...
3. Build the production-grade event-driven fill simulator, including corporate-action cash
   and share effects.
...
- ADR 0007: use exchange-session availability and bitemporal reference data; require
  offline/online feature parity and fail closed where corporate-action execution is absent.
